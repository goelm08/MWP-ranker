...
def do_generate(encoder, decoder, attention_decoder, graph_input, word_manager, form_manager, opt, using_gpu, checkpoint):    
    ...
    if graph_input['g_fw_adj'] == []:
        return "None"
    fw_adj_info = torch.tensor(graph_input['g_fw_adj'], dtype=torch.long)
    bw_adj_info = torch.tensor(graph_input['g_bw_adj'], dtype=torch.long)
    feature_info = torch.tensor(graph_input['g_ids_features'], dtype=torch.long)
    batch_nodes = torch.tensor(graph_input['g_nodes'], dtype=torch.long)
